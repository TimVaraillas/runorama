...
import { RaceStrategyGridComponent } from './race-strategy-grid.component';
...
const meta: Meta<RaceStrategyGridComponent> = {
  title: 'Organisms/NutritionEventGrid',
  component: RaceStrategyGridComponent,
  tags: ['autodocs'],
  argTypes: {
  ...
  render: (args) => ({
    props: args,
    template: `<ui-race-strategy-grid
      [events]="events"
      (select)="select($event)"
      (edit)="edit($event)"
      (delete)="delete($event)"
...
export default meta;
type Story = StoryObj<RaceStrategyGridComponent>;
