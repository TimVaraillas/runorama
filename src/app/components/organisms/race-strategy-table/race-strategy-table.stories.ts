...
const meta: Meta<RaceStrategyTableComponent> = {
  title: 'Organisms/NutritionEventTable',
  component: RaceStrategyTableComponent,
  tags: ['autodocs'],
  argTypes: {
  ...
  render: (args) => ({
    props: args,
    template: `<ui-race-strategy-table
      [events]="events"
      (select)="select($event)"
      (edit)="edit($event)"
      (delete)="delete($event)"
...
export default meta;
type Story = StoryObj<RaceStrategyTableComponent>;
